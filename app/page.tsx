      ...
      <div className="max-w-6xl mx-auto">
       ...
       {/* Header */}
        <div className="text-center mb-8 md:mb-12">
          ...
          <Link 
            href="/leaderboard"
            className="inline-flex items-center gap-2 bg-gradient-to-r from-blue-600 to-blue-800 hover:from-blue-700 hover:to-blue-900 text-white font-bold px-6 py-3 rounded-xl transition transform hover:scale-105 shadow-lg"
          >
            <span>🏆</span>
        ...
        {isConnected && totalScore > 0 && (
          <div className="bg-gradient-to-r from-yellow-400 to-orange-500 rounded-2xl p-6 md:p-8 mb-8 shadow-2xl transform hover:scale-105 transition">
            <div className="text-center">
              <p className="text-white text-lg md:text-xl font-semibold mb-2">
                🏆 Je Totale Score
              </p>
              <p className="text-white text-5xl md:text-7xl font-black">
                {totalScore.toLocaleString('nl-NL', { maximumFractionDigits: 1 })}
              </p>
              <p className="text-white/80 text-sm md:text-base mt-2">punten</p>
            </div>
          </div>
        )}
        ...
        {/* Error Message */}
        {error && (
          <div className="bg-red-500/20 border-2 border-red-500 text-white px-6 py-4 rounded-xl mb-6 backdrop-blur-sm">
        ...
        {syncStats && (
          <div className="bg-green-500/20 border-2 border-green-500 text-white px-6 py-4 rounded-xl mb-6 backdrop-blur-sm">
        ...
        {!isConnected && (
          <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 mb-8 border border-white/20 shadow-2xl">
            ...
            <button
              ...
              className="bg-orange-500 hover:bg-orange-600 text-white font-bold px-8 py-4 rounded-xl flex items-center gap-3 mx-auto transform transition hover:scale-105 shadow-lg"
            >
          ...
          <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-6 mb-8 border border-white/20">
            <div className="flex flex-col md:flex-row items-center justify-between gap-4">
              ...
              <button
                onClick={handleSync}
                disabled={syncing}
                className="bg-blue-500 hover:bg-blue-600 disabled:bg-gray-600 disabled:cursor-not-allowed text-white font-bold px-6 py-3 rounded-xl transform transition hover:scale-105 disabled:hover:scale-100 shadow-lg min-w-[140px]"
              >
                {syncing ? (
        ...
        {runs.length > 0 && (
          <div>
            <h2 className="text-3xl font-bold text-white mb-6 flex items-center gap-3">
              <span>🏃‍♂️ Je Runs</span>
              <span className="text-purple-300 text-xl">({runs.length})</span>
            </h2>
            <div className="space-y-4">
              {runs.map((run) => (
                <div 
                  key={run.id} 
                  className="bg-white/10 backdrop-blur-lg p-6 rounded-xl border border-white/20 hover:bg-white/15 transition-all hover:shadow-xl"
                >
                  <div className="flex flex-col lg:flex-row justify-between items-start gap-4">
                    ...
                    <div className="flex-1 w-full">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-3">
                        <div className="bg-white/5 rounded-lg p-3">
                          <p className="text-purple-300 text-xs mb-1">Afstand</p>
                          <p className="text-white font-bold text-lg">
                            {(run.distance_meters / 1000).toFixed(2)} km
                          </p>
                        </div>
                        <div className="bg-white/5 rounded-lg p-3">
                          <p className="text-purple-300 text-xs mb-1">Tijd</p>
                          <p className="text-white font-bold text-lg">
                        ...
                        {run.average_heartrate && (
                          <div className="bg-white/5 rounded-lg p-3">
                            <p className="text-purple-300 text-xs mb-1">💓 Hartslag</p>
                            <p className="text-white font-bold text-lg">
                    ...
                    <div className="bg-gradient-to-br from-white/10 to-white/5 rounded-xl p-5 border border-white/20 min-w-[250px] w-full lg:w-auto">
                      <p className="text-purple-200 text-sm font-semibold mb-3 text-center">
                        📊 Score Breakdown
                      </p>
                      <div className="space-y-2">
                        <div className="flex justify-between items-center">
                          <span className="text-purple-200 text-sm">📏 Afstand:</span>
                          <span className="text-white font-semibold">
                            +{run.distance_score.toFixed(1)}
                          </span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-purple-200 text-sm">💓 Hartslag:</span>
                          <span className="text-white font-semibold">
                            +{run.heartrate_score.toFixed(1)}
                          </span>
                        </div>
                        {run.consistency_bonus > 0 && (
                          <div className="flex justify-between items-center bg-yellow-500/20 -mx-2 px-2 py-1 rounded">
                            <span className="text-yellow-300 text-sm font-bold">🔥 Bonus:</span>
